...
final class DatePickerMonthBuilderTest extends TestCase
{
    ...
    public function testItBuildsAMonthWithoutAvailableDates(): void
    {
        $calendar = (new DatePickerMonthBuilder())->build(new \DateTimeImmutable('2026-02-01'), []);
        $days = array_merge(...$calendar->weeks);

        self::assertCount(42, $days);
        self::assertCount(0, array_filter($days, static fn ($day): bool => $day->available));
    }

    public function testItIncludesLeapDayWhenBuildingALeapYear(): void
    {
        $calendar = (new DatePickerMonthBuilder())->build(
        ...
        $days = array_merge(...$calendar->weeks);
        $leapDay = array_values(array_filter($days, static fn ($day): bool => '2028-02-29' === $day->date));

        self::assertTrue($leapDay[0]->available);
    }
}
