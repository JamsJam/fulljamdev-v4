...
final class AppointmentReminderDispatcherTest extends TestCase
{
    ...
    public function testItDoesNothingForAnIncompleteAppointment(): void
    {
        $bus = $this->createMock(MessageBusInterface::class);
        $bus->expects(self::never())->method('dispatch');

        (new AppointmentReminderDispatcher($bus, new MockClock()))->dispatch(new Appointment());
    }

    public function testItSkipsRemindersWhoseDateHasPassed(): void
    {
        $bus = $this->createMock(MessageBusInterface::class);
        $bus->expects(self::never())->method('dispatch');
        $appointment = (new Appointment())
            ->setStartAt(new \DateTimeImmutable('2026-08-20 10:00:00 UTC'))
            ->setLink('https://meet.example.test');
        (new \ReflectionProperty(Appointment::class, 'id'))->setValue($appointment, 42);

        (new AppointmentReminderDispatcher($bus, new MockClock('2026-08-20 10:00:00 UTC')))->dispatch($appointment);
    }
}
